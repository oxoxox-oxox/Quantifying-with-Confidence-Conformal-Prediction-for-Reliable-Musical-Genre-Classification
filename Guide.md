...
- **总工期**：建议 4–6 周（全职）。
- **风险**：
  - GTZAN（999 有效文件/10 流派）数据量较小，风格多样性有限 → 备选：FMA 或 MagnaTagATune 补充。
  - ~~共形预测在排序任务中的非一致性分数设计可能不收敛~~ → 已改用方案A(CP分类)，以流派原型质心替代。
  - MERT 嵌入层选择（不同层在不同任务上效果不同）→ 当前仅用最后一层，待后续验证其他层。
  - 覆盖率达标 ✅：CP 在三个 α 水平均紧贴名义值（0.993/0.955/0.910）。
...
*最后更新：2026-07-24 | 阶段：引言完成 (12/19 步骤) | Python 3.11 | PyTorch 2.7+*
